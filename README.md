...
1. When testing my POST route endpoint for comments with a postId, if I purposely used a postId that didn't exist, it resulted in an error: "insert or update on table \"comments\" violates foreign key constraint \"comments_postId_fkey\"". I ended up have to in my catch block, needing to assert in Typescript the error as a DatabaseError and then pass new ClientError using next to call the error middleware.
...
5. I ran into a bug where I would click on a category and it would load the posts from that category, but then if I clicked on the Home link it would only show the posts of the categories that I just navigated from. The problem was my state never got reset so it was still referencing the old category.
...
7. I was making two different API calls in my `App` component to conditionally load posts by all or by category. I ran into an issue where I need to load cards with the posts table, but also join the users table to show the usernames and their respective posts. So I created a type UserPost and then used a union type for my state which would hold the posts retrieved from the API calls.
8. Discovered an issue where all the posts were being rendered in `App`, but since App mounts once and doesn't unmount, a create post action doesn't automatically fetch new posts and update the `App` component. So the post form sends the user to the home page but the new post isn't seen until refreshing the page. I ended up refactoring my code to use context and create a PostsContext provider which held the fetch posts functions and state. Then in my components that needed posts, I destructured my posts object from PostsContext. This also broke my `NavBar` component since it was using an event handler function in App which I moved the fetch posts functions away from `App` to my PostsContext.
9. Issue where creating a post would create the post in the database but the new post wouldn't get fetched on the client side. I had to redo my database schema so that the foreign keys on tables had the exact name for primary keys on other tables. For example users.userId -> post.userId.
10. Ran into an issue where upvoting a post twice would remove from the database but not be updated in the UI. The problem was in my handleUpvote function which would check if the upvote existed. But the problem was if it existed, the two async calls (removeUpvote & checkUpvote) was not being awaited. So the handle function was not waiting for the removeUpvote function to finish. I also made a schema change to the postVotes table and removed postVoteId & totalVotes (weren't necessary) in addition to changing userId and postId to primary keys so there's no way a user can both upvote and downvote a post.
